...
def clip_grad_norm_(
    ...
    This is identical to torch.nn.utils.clip_grad_norm_ except for
        grads += [p.small_grad for p in parameters if hasattr(p, 'small_grad') and (p.small_grad is not None)]
    which handles the approximated gradients of compact.
    We save gradient in small_grad since torch does not allow w and w.grad to have different shapes.
