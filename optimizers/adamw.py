    ...
    def step(self, closure: Callable = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if hasattr(p, 'projector') and (not p.projector.single_gpu):
                    n, rank = p.projector.n, p.projector.rank
                    r = round(n * rank) if rank <= 1 else int(rank)
                    grad = grad[:, :r]
